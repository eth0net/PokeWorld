...
public static class GenPokeBallExplosion
{
    ...
    public static void DoExplosion(
        IntVec3 center, Map map, float radius, DamageDef damType, Thing instigator, float bonusBall, int damAmount = -1,
        float armorPenetration = -1f, SoundDef explosionSound = null, ThingDef weapon = null,
    ...
    )
    {
        ...
        if (armorPenetration < 0f) armorPenetration = damAmount * 0.015f;
        var obj = (PokeBallExplosion)GenSpawn.Spawn(
            DefDatabase<ThingDef>.GetNamed("PW_PokeBallExplosion"), center, map
        );
    ...
    private static void CalculateNeededLOSToCells(
        IntVec3 position, Map map, float direction, out IntVec3? needLOSToCell1, out IntVec3? needLOSToCell2
    )
    {
        needLOSToCell1 = null;
        needLOSToCell2 = null;
        if (position.CanBeSeenOverFast(map)) return;
        direction = GenMath.PositiveMod(direction, 360f);
        var intVec = position;
        intVec.z++;
        var intVec2 = position;
        intVec2.z--;
        var intVec3 = position;
        intVec3.x--;
        var intVec4 = position;
        intVec4.x++;
        if (direction < 90f)
        {
            if (intVec3.InBounds(map) && intVec3.CanBeSeenOverFast(map)) needLOSToCell1 = intVec3;
            if (intVec.InBounds(map) && intVec.CanBeSeenOverFast(map)) needLOSToCell2 = intVec;
        }
        else if (direction < 180f)
        {
            if (intVec.InBounds(map) && intVec.CanBeSeenOverFast(map)) needLOSToCell1 = intVec;
            if (intVec4.InBounds(map) && intVec4.CanBeSeenOverFast(map)) needLOSToCell2 = intVec4;
        }
        else if (direction < 270f)
        {
            if (intVec4.InBounds(map) && intVec4.CanBeSeenOverFast(map)) needLOSToCell1 = intVec4;
            if (intVec2.InBounds(map) && intVec2.CanBeSeenOverFast(map)) needLOSToCell2 = intVec2;
        }
        else
        {
            if (intVec2.InBounds(map) && intVec2.CanBeSeenOverFast(map)) needLOSToCell1 = intVec2;
            if (intVec3.InBounds(map) && intVec3.CanBeSeenOverFast(map)) needLOSToCell2 = intVec3;
        }
    }
