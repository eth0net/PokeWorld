...
namespace PokeWorld
{
    public class PawnKindTable
    {
        ...
        public PawnKindTable(PawnKindTableDef def, Func<IEnumerable<PawnKindDef>> pawnKindsGetter, int uiWidth,
            int uiHeight)
        {
            this.def = def;
        ...
        public void PawnKindTableOnGUI(Vector2 position)
        {
            ...
            var outRect = new Rect((int)position.x, (int)position.y + (int)cachedHeaderHeight, (int)cachedSize.x,
                (int)cachedSize.y - (int)cachedHeaderHeight);
            var viewRect = new Rect(0f, 0f, outRect.width - 16f,
                (int)cachedHeightNoScrollbar - (int)cachedHeaderHeight);
        ...
        private void RecacheColumnWidths_DistributeUntilOptimal(float totalAvailableSpaceForColumns,
            ref float usedWidth, out bool noMoreFreeSpace)
        {
        ...
        private void RecacheColumnWidths_DistributeAboveOptimal(float totalAvailableSpaceForColumns,
            ref float usedWidth)
        {
        ...
        private void RecacheRowHeights()
        {
            ...
            for (var i = 0; i < cachedPawnKinds.Count; i++)
