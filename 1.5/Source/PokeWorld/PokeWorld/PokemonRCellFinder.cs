    ...
    public static class PokemonRCellFinder
    {
        public static IntVec3 BestOrderedGotoDestNear(IntVec3 root, Pawn searcher, Predicate<IntVec3> cellValidator = null)
        {
            ...
            bool predicate(IntVec3 c)
            {
                ...
                {
                    if (thingList[i] is Pawn pawn && pawn != searcher && (pawn.RaceProps.Humanlike) && ((searcher.Faction.IsPlayer && pawn.Faction == searcher.Faction) || (!searcher.Faction.IsPlayer && !pawn.Faction.IsPlayer)))
                    {
                        return false;
                    }
                }
