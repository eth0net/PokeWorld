    ...
    public class CompPokemon : ThingComp
    {
        ...
        public override IEnumerable<StatDrawEntry> SpecialDisplayStats()
        {
            if (parent.Faction != null && parent.Faction == Faction.OfPlayer && statTracker.nature != null)
            {
                ...
                if (nature.increasedStat != nature.decreasedStat)
                    natureDescription +=
                        "PW_StatNatureChangeDesc".Translate(nature.increasedStat.label, nature.decreasedStat.label);
                else
                    natureDescription += "PW_StatNatureNoChangeDesc".Translate();
                yield return new StatDrawEntry(DefDatabase<StatCategoryDef>.GetNamed("PW_PokeWorldStat"),
                    "PW_Nature".Translate(), natureLabel, natureDescription, 4050);
            }
                ...
                if (canEvolve)
                {
                    foreach (var evolution in evolutions)
                    {
                                ...
                                switch (evolution.timeOfDay)
                                {
                                    case TimeOfDay.Day:
                                        evolutionDescription +=
                                            "\n" + "PW_StatEvolutionNeedFixedLevelDay".Translate(text2, evolution.level,
                                                evolutionLabel);
                                        break;
                                    case TimeOfDay.Night:
                                        evolutionDescription +=
                                            "\n" + "PW_StatEvolutionNeedFixedLevelNight".Translate(text2,
                                                evolution.level, evolutionLabel);
                                        break;
                                    default:
                                        evolutionDescription +=
                                            "\n" + "PW_StatEvolutionNeedFixedLevel".Translate(text2, evolution.level,
                                                evolutionLabel);
                                        break;
                                }
                            else
                        ...
                        else if (evolution.requirement == EvolutionRequirement.item)
                        {
                            ...
                            evolutionDescription +=
                                "\n" + "PW_StatEvolutionNeedItem".Translate(Pokemon.KindLabel, itemLabel,
                                    evolutionLabel);
                        }
                    }
                }
            ...
            yield return new StatDrawEntry(DefDatabase<StatCategoryDef>.GetNamed("PW_PokeWorldStat"),
                "PW_EggGroup".Translate(), eggGroupValue, eggGroupDescription, 4049);
            yield return new StatDrawEntry(DefDatabase<StatCategoryDef>.GetNamed("PW_PokeWorldStat"),
                "PW_Type".Translate(), typeValue, typeDescription, 4054);
            yield return new StatDrawEntry(DefDatabase<StatCategoryDef>.GetNamed("PW_PokeWorldStat"),
                "PW_Evolution".Translate(), evolutionValue, evolutionDescription, 4050);
        }
        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            ...
            if (Pokemon.Faction == Faction.OfPlayer)
                if (Pokemon.Map.designationManager.DesignationOn(Pokemon,
                        DefDatabase<DesignationDef>.GetNamed("PW_PutInBall")) == null)
                {
        ...
        public override string CompInspectStringExtra()
        {
            ...
            {
                text = "PW_InspectLevelExperiencePlayerPokemon".Translate(levelTracker.level, levelTracker.experience,
                    levelTracker.totalExpForNextLevel);
