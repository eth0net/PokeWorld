...
internal class WorkGiver_CarryToPortableComputer : WorkGiver_Scanner
{
    ...
    {
        if (pawn.Map.designationManager.DesignationOn(t,
                DefDatabase<DesignationDef>.GetNamed("PW_PutInPortableComputer")) == null) return false;
        if (!pawn.CanReserve(t, 1, 1, null, forced)) return false;
        if (StorageSystem.FindPortableComputerFor(t as CryptosleepBall, pawn) == null) return false;
        return true;
    }

    public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
    {
        return JobMaker.MakeJob(DefDatabase<JobDef>.GetNamed("PW_CarryToPortableComputer"), t,
            StorageSystem.FindPortableComputerFor(t as CryptosleepBall, pawn));
    }
}
