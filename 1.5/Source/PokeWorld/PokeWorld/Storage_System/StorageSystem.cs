...
public class StorageSystem : WorldComponent, ISuspendableThingHolder
{
    ...
    public virtual bool TryAcceptThing(CryptosleepBall cryptosleepBall, bool allowSpecialEffects = true)
    {
        ...
        {
            cryptosleepBall.innerContainer.TryTransferToContainer(cryptosleepBall.ContainedThing, innerContainer,
                cryptosleepBall.stackCount);
    ...
    public static Building_PortableComputer FindPortableComputerFor(CryptosleepBall p, Pawn traveler,
        bool ignoreOtherReservations = false)
    {
        foreach (var item in DefDatabase<ThingDef>.AllDefs.Where(def =>
                     typeof(Building_PortableComputer).IsAssignableFrom(def.thingClass)))
        {
            var building_PortableComputer = (Building_PortableComputer)GenClosest.ClosestThingReachable(p.Position,
                p.Map, ThingRequest.ForDef(item), PathEndMode.InteractionCell, TraverseParms.For(traveler));
            if (building_PortableComputer != null &&
                building_PortableComputer.TryGetComp<CompPowerTrader>().PowerOn &&
