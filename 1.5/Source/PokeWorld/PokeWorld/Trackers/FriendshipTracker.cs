...
namespace PokeWorld
{
    public class FriendshipTracker : IExposable
    {
            ...
            if (pokemonHolder.Faction == Faction.OfPlayer)
            {
                ...
                if (friendship >= maxFriendship)
                {
                    ...
                    if (!flagMaxFriendshipMessage)
                    {
                        ...
                        Messages.Message("PW_PokemonLovesYou".Translate(pokemonHolder.Label), pokemonHolder,
        ...
        public string GetStatement()
        {
            string text;
            if (friendship >= 250)
                text = "PW_FriendshipStatementReallyHappy".Translate(
                    pokemonHolder.gender.GetPronoun().CapitalizeFirst());
