...
public class ShinyTracker : IExposable
{
    ...
    public void ShinyTickRare()
    {
        if (isShiny)
        {
            ...
            {
                Letter letter = LetterMaker.MakeLetter("PW_ShinyPokemonLetter".Translate(),
                    "PW_ShinyPokemonLetterDesc".Translate(), LetterDefOf.PositiveEvent, pokemonHolder);
    ...
    public void TryMakeShinyMote()
    {
        if (pokemonHolder.Spawned && !pokemonHolder.Position.Fogged(pokemonHolder.Map))
        {
            ...
            for (var i = 0; i < num; i++)
                ThrowShinyIcon(pokemonHolder.Position, pokemonHolder.Map,
                    DefDatabase<ThingDef>.GetNamed("Mote_ShinyStar"));
        }
    }
