    ...
    public static void Postfix(Func<Type, string, Verb> __0, VerbTracker __instance)
    {
        ...
        foreach (var kvp in comp.moveTracker.unlockableMoves)
        {
            ...
            if (verbProperties != null)
                try
                {
                    var text = Verb.CalculateUniqueLoadID(pawn,
                        comp.moveTracker.unlockableMoves.Keys.ToList().IndexOf(kvp.Key));

                    var methodInitVerb = __instance.GetType()
                        .GetMethod("InitVerb", BindingFlags.NonPublic | BindingFlags.Instance);
                    methodInitVerb.Invoke(__instance,
            ...
            if (tool != null)
            {
                ...
                try
                {
                    ...
                    var text2 = Verb.CalculateUniqueLoadID(pawn,
                        comp.moveTracker.unlockableMoves.Keys.ToList().IndexOf(kvp.Key));

                    var methodInitVerb = __instance.GetType()
                        .GetMethod("InitVerb", BindingFlags.NonPublic | BindingFlags.Instance);
                    methodInitVerb.Invoke(__instance,
                        new object[] { __0(verb.verbClass, text2), verb, tool, maneuver, text2 });
                }
                catch (Exception ex2)
                {
