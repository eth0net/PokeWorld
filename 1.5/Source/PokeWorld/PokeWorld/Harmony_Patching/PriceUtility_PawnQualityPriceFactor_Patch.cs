...
internal class PriceUtility_PawnQualityPriceFactor_Patch
{
    public static void Postfix(Pawn __0, StringBuilder __1, ref float __result)
    {
        ...
        if (comp != null)
        {
            ...
            __1?.AppendLine("   " + "PW_PriceFactorLevel"
                .Translate(comp.levelTracker.level, levelFactor.ToStringPercent()).ToLower().CapitalizeFirst());
            __result *= levelFactor;
            if (comp.shinyTracker.isShiny)
            {
                float shinyFactor = 4;
                __1?.AppendLine("   " + "PW_PriceFactorShiny".Translate(shinyFactor.ToStringPercent()).ToLower()
                    .CapitalizeFirst());
