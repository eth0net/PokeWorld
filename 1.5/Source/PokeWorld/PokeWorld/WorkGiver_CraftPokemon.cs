...
namespace PokeWorld
{
    internal class WorkGiver_CraftPokemon : WorkGiver_Scanner
    {
        ...
        private static Job FinishUftJob(Pawn pawn, UnfinishedThing uft, Bill_ProductionWithUft bill)
        {
            if (uft.Creator != pawn)
            {
                Log.Error(string.Concat("Tried to get FinishUftJob for ", pawn, " finishing ", uft,
                    " but its creator is ", uft.Creator));
                return null;
            }
            ...
            var job2 = JobMaker.MakeJob(DefDatabase<JobDef>.GetNamed("PW_CraftPokemon"),
                (Thing)bill.billStack.billGiver);
            job2.bill = bill;
                    ...
                    if (bill_ProductionWithUft.BoundUft != null)
                    {
                        if (bill_ProductionWithUft.BoundWorker == pawn &&
                            pawn.CanReserveAndReach(bill_ProductionWithUft.BoundUft, PathEndMode.Touch,
                                Danger.Deadly) && !bill_ProductionWithUft.BoundUft.IsForbidden(pawn))
        ...
        public static Job TryStartNewCraftPokemonJob(Pawn pawn, Bill bill, IBillGiver giver,
            List<ThingCount> chosenIngThings, out Job haulOffJob, bool dontCreateJobIfHaulOffRequired = true)
        {
        ...
        public static bool TryFindBestFixedIngredients(List<IngredientCount> ingredients, Pawn pawn,
            Thing ingredientDestination, List<ThingCount> chosen, float searchRadius = 999f)
        {
            return TryFindBestIngredientsHelper(t => ingredients.Any(ingNeed => ingNeed.filter.Allows(t)),
                foundThings => TryFindBestIngredientsInSet_NoMixHelper(foundThings, ingredients, chosen,
                    GetBillGiverRootCell(ingredientDestination, pawn), false), ingredients, pawn, ingredientDestination,
                chosen, searchRadius);
        }
        ...
        private static bool TryFindBestBillIngredients(Bill bill, Pawn pawn, Thing billGiver, List<ThingCount> chosen)
        {
            return TryFindBestIngredientsHelper(
                t => bill.IsFixedOrAllowedIngredient(t) &&
                     bill.recipe.ingredients.Any(ingNeed => ingNeed.filter.Allows(t)),
                foundThings => TryFindBestBillIngredientsInSet(foundThings, bill, chosen,
                    GetBillGiverRootCell(billGiver, pawn), billGiver is Pawn), bill.recipe.ingredients, pawn, billGiver,
                chosen, bill.ingredientSearchRadius);
        }

        private static bool TryFindBestIngredientsHelper(Predicate<Thing> thingValidator,
        ...
        private static IntVec3 GetBillGiverRootCell(Thing billGiver, Pawn forPawn)
        {
            if (billGiver is Building building)
            {
                ...
                Log.Error(string.Concat("Tried to find bill ingredients for ", billGiver,
        ...
        private static void AddEveryMedicineToRelevantThings(Pawn pawn, Thing billGiver, List<Thing> relevantThings,
            Predicate<Thing> baseValidator, Map map)
        {
            ...
            tmpMedicine.SortBy(x => 0f - x.GetStatValue(StatDefOf.MedicalPotency),
        ...
        private static bool TryFindBestBillIngredientsInSet_NoMix(List<Thing> availableThings, Bill bill,
            List<ThingCount> chosen, IntVec3 rootCell, bool alreadySorted)
        {
            return TryFindBestIngredientsInSet_NoMixHelper(availableThings, bill.recipe.ingredients, chosen, rootCell,
                alreadySorted, bill);
        }

        private static bool TryFindBestIngredientsInSet_NoMixHelper(List<Thing> availableThings,
            ...
            for (var i = 0; i < ingredients.Count; i++)
            {
                ...
                for (var j = 0; j < availableCounts.Count; j++)
                {
                    ...
                    if ((bill != null && !bill.recipe.ignoreIngredientCountTakeEntireStacks &&
                         num > availableCounts.GetCount(j)) ||
                        !ingredientCount.filter.Allows(availableCounts.GetDef(j)) || (bill != null &&
                            !ingredientCount.IsFixedIngredient &&
        ...
        private static bool TryFindBestBillIngredientsInSet_AllowMix(List<Thing> availableThings, Bill bill,
            List<ThingCount> chosen, IntVec3 rootCell)
        {
            chosen.Clear();
            availableThings.SortBy(t => bill.recipe.IngredientValueGetter.ValuePerUnitOf(t.def),
