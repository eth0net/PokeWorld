...
{
    public static Pawn GenerateAndSpawnNewPokemon(PawnKindDef kind, Faction faction, IntVec3 position, Map map,
        Pawn trainer = null, bool newBorn = false, bool fossil = false)
    {
        var Pokemon = PawnGenerator.GeneratePawn(kind, faction);
        if (newBorn)
        {
