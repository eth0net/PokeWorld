...
{
    [TweakValue("LogFilter", 0f, 1f)] private static readonly float DisplayChanceOnMiss = 0.25f;
    ...
    public BattleLogEntry_PokemonRangedMoveImpact(Thing initiator, Thing recipient, Thing originalTarget,
        MoveDef moveDef, ThingDef projectileDef, ThingDef coverDef)
    {
        ...
        if (originalTarget is Pawn)
        {
            ...
            originalTargetMobile =
                !originalTargetPawn.Downed && !originalTargetPawn.Dead && originalTargetPawn.Awake();
        }
    ...
    public override GrammarRequest GenerateGrammarRequest()
    {
        ...
        if (originalTargetPawn != recipientPawn || originalTargetThing != recipientThing)
        {
            ...
            {
                result.Rules.AddRange(GrammarUtility.RulesForPawn("ORIGINALTARGET", originalTargetPawn,
                    result.Constants));
