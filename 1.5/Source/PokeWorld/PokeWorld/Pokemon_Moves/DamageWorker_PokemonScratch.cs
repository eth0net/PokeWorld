...
internal class DamageWorker_PokemonScratch : DamageWorker_PokemonMeleeMove
{
        ...
        DamageResult result)
    {
        ...
        enumerable = enumerable.Where(target =>
            target != dinfo.HitPart && !target.def.conceptual && target.depth == BodyPartDepth.Outside &&
        ...
        FinalizeAndAddInjury(pawn,
            ReduceDamageToPreserveOutsideParts(totalDamage * def.scratchSplitPercentage, dinfo, pawn), dinfo,
            result);
        var dinfo3 = dinfo;
        dinfo3.SetHitPart(bodyPartRecord2);
        FinalizeAndAddInjury(pawn,
            ReduceDamageToPreserveOutsideParts(totalDamage * def.scratchSplitPercentage, dinfo3, pawn), dinfo3,
            result);
    }
}
