...
public struct PokeBallShotReport
{
    ...
    private float FactorFromPosture
    {
        get
        {
            if (target.HasThing && target.Thing is Pawn p && distance >= 4.5f && p.GetPosture() != 0) return 0.2f;
            return 1f;
        }
    }
    ...
    private float FactorFromExecution
    {
        get
        {
            if (target.HasThing && target.Thing is Pawn p && distance <= 3.9f && p.GetPosture() != 0) return 7.5f;
            return 1f;
        }
    }
    ...
    public float AimOnTargetChance_StandardTarget
    {
        get
        {
            var num = factorFromShooterAndDist * factorFromEquipment * factorFromWeather * factorFromCoveringGas *
                      FactorFromExecution;
            num += offsetFromDarkness;
            if (num < 0.0201f) num = 0.0201f;
            return num;
        }
    }
    ...
    public static PokeBallShotReport HitReportFor(Thing caster, Verb verb, LocalTargetInfo target)
    {
        ...
        var result = default(PokeBallShotReport);
        ...
        result.coversOverallBlockChance =
            CoverUtility.CalculateOverallBlockChance(target, caster.Position, caster.Map);
        result.factorFromCoveringGas = 1f;
        if (verb.TryFindShootLineFromTo(verb.caster.Position, target, out result.shootLine))
        {
            foreach (var item in result.shootLine.Points())
        ...
        if (target.HasThing)
        {
            if (target.Thing is Pawn pawn)
                ...
                result.factorFromTargetSize = target.Thing.def.fillPercent * target.Thing.def.size.x *
    ...
    {
        return HitFactorFromShooter(
            caster is Pawn
                ? caster.GetStatValue(DefDatabase<StatDef>.GetNamed("PW_PokeBallAccuracyPawn"))
                : caster.GetStatValue(StatDefOf.ShootingAccuracyTurret), distance);
    }
    ...
    public string GetTextReadout()
    {
        ...
        if (forcedMissRadius > 0.5f)
        {
            ...
            stringBuilder.AppendLine("DirectHitChance".Translate() + ": " +
                                     (1f / GenRadial.NumCellsInRadius(forcedMissRadius)).ToStringPercent());
        }
        else
        {
            ...
            stringBuilder.AppendLine("   " + "ShootReportWeapon".Translate() + ": " +
                                     factorFromEquipment.ToStringPercent());
            if (target.HasThing && factorFromTargetSize != 1f)
                stringBuilder.AppendLine("   " + "TargetSize".Translate() + ": " +
                                         factorFromTargetSize.ToStringPercent());
            if (factorFromWeather < 0.99f)
                stringBuilder.AppendLine("   " + "Weather".Translate() + ": " +
                                         factorFromWeather.ToStringPercent());
            if (factorFromCoveringGas < 0.99f)
                stringBuilder.AppendLine("   " + "BlindSmoke".Translate().CapitalizeFirst() + ": " +
                                         factorFromCoveringGas.ToStringPercent());
            if (FactorFromPosture < 0.9999f)
                stringBuilder.AppendLine("   " + "TargetProne".Translate() + ": " +
                                         FactorFromPosture.ToStringPercent());
            if (FactorFromExecution != 1f)
                stringBuilder.AppendLine("   " + "Execution".Translate() + ": " +
                                         FactorFromExecution.ToStringPercent());
            if (ModsConfig.IdeologyActive && target.HasThing && offsetFromDarkness != 0f)
            {
                if (DarknessCombatUtility.IsOutdoorsAndLit(target.Thing))
                    stringBuilder.AppendLine("   " + StatDefOf.ShootingAccuracyOutdoorsLitOffset.LabelCap + ": " +
                                             offsetFromDarkness.ToStringPercent());
                else if (DarknessCombatUtility.IsOutdoorsAndDark(target.Thing))
                    stringBuilder.AppendLine("   " + StatDefOf.ShootingAccuracyOutdoorsDarkOffset.LabelCap + ": " +
                                             offsetFromDarkness.ToStringPercent());
                else if (DarknessCombatUtility.IsIndoorsAndDark(target.Thing))
                    stringBuilder.AppendLine("   " + StatDefOf.ShootingAccuracyIndoorsDarkOffset.LabelCap + ": " +
                                             offsetFromDarkness.ToStringPercent());
                else if (DarknessCombatUtility.IsIndoorsAndLit(target.Thing))
                    stringBuilder.AppendLine("   " + StatDefOf.ShootingAccuracyIndoorsLitOffset.LabelCap + "   " +
                                             offsetFromDarkness.ToStringPercent());
            }

            if (PassCoverChance < 1f)
            {
                stringBuilder.AppendLine("   " + "ShootingCover".Translate() + ": " +
                                         PassCoverChance.ToStringPercent());
