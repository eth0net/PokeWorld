...
internal class DamageWorker_TryCatch : DamageWorker
{
    ...
    {
        if (thing.GetType() != typeof(Pawn)) return base.Apply(dinfo, thing);
        ...
        if (pawn != targetPawn) return base.Apply(dinfo, thing);
        var compPokemon = pawn.TryGetComp<CompPokemon>();
        if (pawn.AnimalOrWildMan() &&
            (pawn.Faction == null || pawn.Faction ==
                Find.FactionManager.AllFactions.First(f => f.def.defName == "PW_HostilePokemon")) &&
            compPokemon != null &&
            ...
            if (catchRate > 0)
            {
                ...
                else
                {
                    ...
                    if (pawn.Downed)
                    {
                        ...
                        {
                            pawn.Kill(dinfo);
                            Messages.Message("PW_MessagePokemonCatchDied".Translate(pawn.KindLabel),
                                pawn.Corpse, MessageTypeDefOf.NegativeEvent);
                        }
                        else
                        {
                    ...
                    else if (Rand.Chance(pawn.RaceProps.manhunterOnTameFailChance))
                    {
                        ...
                        {
                            string text2 = "AnimalManhunterFromTaming"
                                .Translate(pawn.Label, pawn.Named("PAWN")).AdjustedFor(pawn);
                            ...
                            if (Find.Storyteller.difficulty.allowBigThreats && Rand.Value < 0.5f)
                            {
                                ...
                                num += raceMates
                                    .Where(t => pawn != t && t.def == pawn.def && t.Faction == pawn.Faction &&
                                                t.Position.InHorDistOf(pawn.Position, 24f) && t.GetRoom() == pawnRoom)
                                    .Count(t =>
                                        t.mindState.mentalStateHandler.TryStartMentalState(MentalStateDefOf.Manhunter));
                            ...
                            string str = "LetterLabelAnimalManhunterRevenge".Translate(value)
                                .CapitalizeFirst();
                            Find.LetterStack.ReceiveLetter(str, text2,
                                num == 1 ? LetterDefOf.ThreatSmall : LetterDefOf.ThreatBig, target);
                        }
                    }
