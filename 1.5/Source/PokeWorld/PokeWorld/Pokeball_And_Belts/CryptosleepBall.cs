...
public class CryptosleepBall : ThingWithComps, ISuspendableThingHolder, IOpenable
{
            ...
            if (innerContainer.First() is Pawn pokemon)
            {
                ...
                if (comp != null)
                {
                    ...
                    if (pokemon.gender != Gender.None)
                        return text + "PW_PokeballContainsLongGendered".Translate(str.CapitalizeFirst(), level,
                            pokemon.gender.ToString().ToLower(), species);
                    return text +
    ...
    public override IEnumerable<Gizmo> GetGizmos()
    {
        ...
        if (DefDatabase<ResearchProjectDef>.GetNamed("PW_StorageSystem").IsFinished &&
            ContainedThing is Pawn pawn && pawn.Faction == Faction.OfPlayer)
            if (Map.designationManager.DesignationOn(this,
                    DefDatabase<DesignationDef>.GetNamed("PW_PutInPortableComputer")) == null)
            {
