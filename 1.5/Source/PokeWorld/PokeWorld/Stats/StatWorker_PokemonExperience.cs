...
public class StatWorker_PokemonExperience : StatWorker
{
        ...
        float finalVal)
    {
        if (req.Thing != null)
        {
            ...
            stringBuilder.AppendLine("PW_StatExperienceDesc".Translate(
                pawn.TryGetComp<CompPokemon>().levelTracker.experience,
                pawn.TryGetComp<CompPokemon>().levelTracker.totalExpForNextLevel.ToString()));
            stringBuilder.AppendLine();
            stringBuilder.AppendLine("PW_StatExperienceProgress".Translate(
                (pawn.TryGetComp<CompPokemon>().levelTracker.experience /
                 (float)pawn.TryGetComp<CompPokemon>().levelTracker.totalExpForNextLevel).ToStringPercent()));
