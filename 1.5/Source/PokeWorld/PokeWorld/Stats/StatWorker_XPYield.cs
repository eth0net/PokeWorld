...
internal class StatWorker_XPYield : StatWorker
{
    public override float GetValueUnfinalized(StatRequest req, bool applyPostProcess = true)
    {
        ...
        if (req.HasThing && req.Thing is Pawn)
        {
            ...
            var valueUnfinalized =
                base.GetValueUnfinalized(StatRequest.For(req.BuildableDef, req.StuffDef), applyPostProcess);
            if (comp != null)
            {
            ...
            else
            {
                ...
                value += (int)(300 * (GetOverallArmor(pawn, StatDefOf.ArmorRating_Blunt) +
                                      ...
                                      GetOverallArmor(pawn, StatDefOf.ArmorRating_Sharp)));
        ...
        if (req.HasThing && req.Thing is Pawn)
        {
            ...
            if (comp != null)
            {
                stringBuilder.AppendLine("PW_StatLevel"
                    .Translate(comp.levelTracker.level.ToString(), (comp.levelTracker.level / 7f).ToStringPercent())
                    .ToLower().CapitalizeFirst());
            }
            else if (pawn.AnimalOrWildMan() && pawn.kindDef != PawnKindDefOf.WildMan)
            {
                stringBuilder.AppendLine("PW_StatXPYieldCombatPower".Translate(
            ...
            else
            {
                ...
                stringBuilder.AppendLine("PW_StatXPYieldApparel".Translate(
                    ((int)(300 * (GetOverallArmor(pawn, StatDefOf.ArmorRating_Blunt) +
                                  GetOverallArmor(pawn, StatDefOf.ArmorRating_Heat) +
                                  GetOverallArmor(pawn, StatDefOf.ArmorRating_Sharp)))).ToString()));
                if (pawn.equipment != null && pawn.equipment.Primary != null &&
                    pawn.equipment.Primary.def.IsRangedWeapon)
                    stringBuilder.AppendLine("PW_StatXPYieldWeapon".Translate(Mathf
                        .Min((int)(pawn.equipment.Primary.MarketValue * 0.8f), 1000).ToString()));
                else
                    stringBuilder.AppendLine("PW_StatXPYieldMeleeDPS".Translate(Mathf
                        .Min((int)(pawn.GetStatValue(StatDefOf.MeleeDPS) * 40), 1000).ToString()));
            }
        }
