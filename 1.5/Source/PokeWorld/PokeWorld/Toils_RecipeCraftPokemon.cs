...
public static class Toils_RecipeCraftPokemon
{
    ...
    public static Toil MakeUnfinishedThingIfNeeded()
    {
        ...
        toil.initAction = delegate
        {
            ...
            if (curJob.RecipeDef.UsesUnfinishedThing && !(curJob.GetTarget(TargetIndex.B).Thing is UnfinishedThing))
            {
                ...
                var unfinishedThing =
                    (UnfinishedThing)ThingMaker.MakeThing(curJob.RecipeDef.unfinishedThingDef, stuff);
    ...
    public static void FinishRecipeAndSpawnPokemonAction(Toil toil)
    {
        ...
        {
            var xp = jobDriver_CraftPokemon.ticksSpentDoingRecipeWork * 0.1f *
                     curJob.RecipeDef.workSkillLearnFactor;
        ...
        var list = GenRecipe.MakeRecipeProducts(curJob.RecipeDef, actor, ingredients, dominantIngredient,
            jobDriver_CraftPokemon.BillGiver).ToList();
        ...
        if (curJob.RecipeDef.products[0].thingDef.HasComp(typeof(CompPokemon)) && curJob.RecipeDef.products[0]
                .thingDef.GetCompProperties<CompProperties_Pokemon>().attributes.Contains(PokemonAttribute.Fossil))
        {
        ...
        Find.World.GetComponent<PokedexManager>()
            .AddPokemonKindCaught(DefDatabase<PawnKindDef>.GetNamed(curJob.RecipeDef.products[0].thingDef.defName));
        Messages.Message(message.Translate(actor.LabelShortCap, revivedPokemon.KindLabel), revivedPokemon,
    ...
    private static List<Thing> CalculateIngredients(Job job, Pawn actor)
    {
        ...
        if (job.placedThings != null)
            for (var i = 0; i < job.placedThings.Count; i++)
            {
                ...
                {
                    Log.Error(string.Concat("PlacedThing ", job.placedThings[i], " with count ",
                        job.placedThings[i].Count, " for job ", job));
                    continue;
                }
    ...
    public static Toil PlaceHauledThingInCell(TargetIndex cellInd, Toil nextToilOnPlaceFailOrIncomplete,
        bool storageMode, bool tryStoreInSameStorageIfSpotCantHoldWholeStack = false)
    {
        var toil = new Toil();
        toil.initAction = delegate
        {
            ...
            else
            {
                ...
                if (slotGroup != null && slotGroup.Settings.AllowedToAccept(actor.carryTracker.CarriedThing))
                    actor.Map.designationManager.TryRemoveDesignationOn(actor.carryTracker.CarriedThing,
                        DesignationDefOf.Haul);
                Action<Thing, int> placedAction = null;
                if (curJob.def == JobDefOf.DoBill ||
                    curJob.def == DefDatabase<JobDef>.GetNamed("PW_CraftPokemon") ||
                    ...
                    if (storageMode)
                    {
                        if (nextToilOnPlaceFailOrIncomplete != null &&
                            ((tryStoreInSameStorageIfSpotCantHoldWholeStack &&
                              StoreUtility.TryFindBestBetterStoreCellForIn(actor.carryTracker.CarriedThing, actor,
                                  actor.Map, StoragePriority.Unstored, actor.Faction, cell.GetSlotGroup(actor.Map),
                                  out var foundCell)) ||
                             StoreUtility.TryFindBestBetterStoreCellFor(actor.carryTracker.CarriedThing, actor,
                                 actor.Map, StoragePriority.Unstored, actor.Faction, out foundCell)))
                        {
                        ...
                        else
                        {
                            ...
                            {
                                Log.Error(string.Concat("Incomplete haul for ", actor,
                                    ": Could not find anywhere to put ", actor.carryTracker.CarriedThing, " near ",
                                    actor.Position, ". Destroying. This should never happen!"));
