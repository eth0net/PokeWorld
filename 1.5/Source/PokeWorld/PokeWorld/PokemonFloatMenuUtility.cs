...
namespace PokeWorld
{
    public static class PokemonFloatMenuUtility
    {
        public static Action GetRangedAttackAction(Pawn pawn, LocalTargetInfo target, out string failStr)
        {
            ...
            {
                if (!(target.Thing is Pawn target2) || (!pawn.InSameExtraFaction(target2, ExtraFactionType.HomeFaction) && !pawn.InSameExtraFaction(target2, ExtraFactionType.MiniFaction)))
                {
                    return delegate
                    {
        ...
        public static FloatMenuOption DecoratePrioritizedTask(FloatMenuOption option, Pawn pawn, LocalTargetInfo target, string reservedText = "ReservedBy")
        {
            ...
            {
                Pawn pawn2 = pawn.Map.reservationManager.FirstRespectedReserver(target, pawn) ?? pawn.Map.physicalInteractionReservationManager.FirstReserverOf(target);
                if (pawn2 != null)
                {
