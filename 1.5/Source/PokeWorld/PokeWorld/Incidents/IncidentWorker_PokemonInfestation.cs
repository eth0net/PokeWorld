...
namespace PokeWorld
{
    public class IncidentWorker_PokemonInfestation : IncidentWorker
    {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            if (PokeWorldSettings.OkforPokemon() && PokeWorldSettings.allowPokemonInfestation)
            {
                ...
                var t = PokemonInfestationUtility.SpawnTunnels(hiveDef,
                    Mathf.Max(GenMath.RoundRandom(parms.points / 220f), 1), map);
    ...
    {
        public static TunnelPokemonHiveSpawner SpawnTunnels(ThingDef hiveDef, int hiveCount, Map map,
            bool spawnAnywhereIfNoGoodCell = false, bool ignoreRoofedRequirement = false, string questTag = null)
        {
        ...
        {
            var def = DefDatabase<ThingDef>.AllDefs.Where(x => x.defName == "PW_PokemonHiveGeodude"
        ...
        {
            var def = DefDatabase<ThingDef>.AllDefs.Where(x => x.defName == "PW_InsectPokemonHive"
    ...
    public class TunnelPokemonHiveSpawner : ThingWithComps
    {
        ...
        public override void Tick()
        {
            ...
            insectsPoints = Mathf.Max(insectsPoints,
                pokemonHiveDef.GetCompProperties<CompProperties_SpawnerPawn>().spawnablePawnKinds
                    .Min(x => x.combatPower));
            ...
            for (; pointsLeft > 0f; pointsLeft -= result2.combatPower)
            {
                ...
                var pawn = PawnGenerator.GeneratePawn(result2,
