...
internal class IncidentWorker_PokemonManhunterPack : IncidentWorker
{
    ...
    public override bool TryExecuteWorker(IncidentParms parms)
    {
        ...
        if (!result.IsValid &&
            !RCellFinder.TryFindRandomPawnEntryCell(out result, map, CellFinder.EdgeRoadChance_Animal))
            return false;
        if (animalKind[0].race.HasComp(typeof(CompPokemon)))
        {
            var list = Pokemon_ManhunterPackIncidentUtility.GeneratePokemonFamily_NewTmp(animalKind, map.Tile,
                parms.points * PointsFactor, parms.pawnCount);
            ...
            SendStandardLetter("LetterLabelManhunterPackArrived".Translate(),
                "ManhunterPackArrived".Translate(animalKind[0].GetLabelPlural()), LetterDefOf.ThreatBig, parms,
                list[0]);
...
public static class Pokemon_ManhunterPackIncidentUtility
{
    ...
    public static bool TryFindManhunterAnimalKind(float points, int tile, out List<PawnKindDef> allPokemonKind)
    {
        if (PokeWorldSettings.OkforPokemon())
        {
            var source = DefDatabase<PawnKindDef>.AllDefs.Where(k =>
                ...
                PokeWorldSettings.GenerationAllowed(k.race.GetCompProperties<CompProperties_Pokemon>()
                    .generation) && k.canArriveManhunter && (tile == -1 ||
            ...
            if (source.Any())
            {
                ...
                allPokemonKind = DefDatabase<PawnKindDef>.AllDefs.Where(x =>
                    x.RaceProps.Animal && x.canArriveManhunter && x.race.HasComp(typeof(CompPokemon)) &&
                    (x.race.comps.Find(y => y.compClass == typeof(CompPokemon)) as CompProperties_Pokemon)
                    .evolutionLine == evolutionLine &&
                    (tile == -1 ||
                     Find.World.tileTemperatures.SeasonAndOutdoorTemperatureAcceptableFor(tile, x.race))).ToList();
                return true;
            }
        }
    ...
    public static List<Pawn> GeneratePokemonFamily_NewTmp(List<PawnKindDef> allPokemonKind, int tile, float points,
        int animalCount = 0)
    {
        ...
        for (; pointsLeft > 0f; pointsLeft -= result2.combatPower)
        {
            ...
            var pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(result2, null,
    ...
    public static void ManhunterResults()
    {
        ...
        DebugTables.MakeTablesDialog(list, points => points.ToString("F0") + " pts", candidates,
            ...
            delegate(float points, PawnKindDef candidate)
            {
                ...
                return num2 == 0f
                    ? "0%"
                    : string.Format("{0}%, {1}", (num2 * 100f / num).ToString("F0"),
