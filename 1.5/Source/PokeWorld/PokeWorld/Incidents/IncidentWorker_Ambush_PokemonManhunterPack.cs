...
internal class IncidentWorker_Ambush_PokemonManhunterPack : IncidentWorker_Ambush
{
    ...
    protected override bool CanFireNowSub(IncidentParms parms)
    {
        ...
        return Pokemon_ManhunterPackIncidentUtility.TryFindManhunterAnimalKind(
            AdjustedPoints(parms.points), -1, out animalKind
        );
    }

    protected override List<Pawn> GeneratePawns(IncidentParms parms)
    {
        if (!Pokemon_ManhunterPackIncidentUtility.TryFindManhunterAnimalKind(
                AdjustedPoints(parms.points), parms.target.Tile, out var animalKind
        ...
        return Pokemon_ManhunterPackIncidentUtility.GeneratePokemonFamily_NewTmp(
            animalKind, parms.target.Tile, AdjustedPoints(parms.points)
        );
    }
    ...
    protected override string GetLetterText(Pawn anyPawn, IncidentParms parms)
    {
        ...
        return string.Format(
            def.letterText, caravan != null ? caravan.Name : "yourCaravan".TranslateSimple(),
            anyPawn.GetKindLabelPlural()
        ).CapitalizeFirst();
    }
}
