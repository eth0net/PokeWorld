...
namespace PokeWorld
{
    internal class IncidentWorker_PokemonHerdMigration : IncidentWorker
    {
        ...
        private bool TryFindAnimalKind(int tile, out List<PawnKindDef> allPokemonKind)
        {
            ...
            if (source.Any())
            {
                if (source.TryRandomElementByWeight(x => Mathf.Lerp(0.2f, 1f, x.RaceProps.wildness),
                        out singlePokemonKind))
                {
                }
                else
                {
                ...
                allPokemonKind = DefDatabase<PawnKindDef>.AllDefs.Where(x =>
                        x.RaceProps.Animal && x.RaceProps.CanDoHerdMigration && x.race.HasComp(typeof(CompPokemon)) &&
        ...
        private bool TryFindStartAndEndCells(Map map, out IntVec3 start, out IntVec3 end)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                if (!CellFinder.TryFindRandomEdgeCellWith(
                        x => map.reachability.CanReach(startLocal, x, PathEndMode.OnCell,
                            TraverseMode.NoPassClosedDoors, Danger.Deadly), map, CellFinder.EdgeRoadChance_Ignore,
                        out var result)) break;
        ...
        private List<Pawn> GenerateAnimals(List<PawnKindDef> allPokemonKind, int tile)
        {
            ...
            for (var i = 0; i < allPokemonKind.Count; i++)
            {
                ...
                randomInRange = Mathf.Max(randomInRange,
                    Mathf.CeilToInt(4f / (allPokemonKind[i].RaceProps.baseBodySize * allPokemonKind.Count)));

                for (var j = 0; j < randomInRange; j++)
                {
                    var item = PawnGenerator.GeneratePawn(new PawnGenerationRequest(allPokemonKind[i], null,
