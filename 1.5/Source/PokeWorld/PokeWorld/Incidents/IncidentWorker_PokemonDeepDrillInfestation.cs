...
public class IncidentWorker_PokemonDeepDrillInfestation : IncidentWorker
{
    ...
    {
        if (!PokeWorldSettings.OkforPokemon() || !PokeWorldSettings.allowPokemonInfestation) return false;
        var map = (Map)parms.target;
        tmpDrills.Clear();
        DeepDrillInfestationIncidentUtility.GetUsableDeepDrills(map, tmpDrills);
        ...
        var intVec = CellFinder.FindNoWipeSpawnLocNear(deepDrill.Position, map,
            DefDatabase<ThingDef>.GetNamed("PW_TunnelPokemonHiveSpawner"), Rot4.North, 2,
            x => x.Walkable(map) && x.GetFirstThing(map, deepDrill.def) == null &&
                 x.GetFirstThingWithComp<CompCreatesInfestations>(map) == null &&
                 x.GetFirstThing(map, hiveDef) == null && x.GetFirstThing(map,
